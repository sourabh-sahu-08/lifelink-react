                ...
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
                    ...
                    <motion.div initial={{ scale: 0.9, y: 20, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.9, y: 20, opacity: 0 }} className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 z-10">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h2 className="text-2xl font-bold text-gray-900">Post Blood Supply</h2>
                                <p className="text-xs text-gray-400 font-semibold uppercase tracking-widest mt-1">Offer available blood to the network</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Blood Type Available *</label>
                                <div className="grid grid-cols-4 gap-2">
                                    {BLOOD_TYPES.map(bt => (
                                        <button type="button" key={bt} onClick={() => setForm(p => ({ ...p, bloodType: bt }))}
                                            className={`py-3 rounded-2xl font-semibold text-sm border-2 transition-all ${form.bloodType === bt ? 'bg-blue-600 text-white border-blue-600 shadow-lg shadow-blue-200' : 'border-gray-100 text-gray-500 hover:border-blue-300'}`}>
                                            {bt}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Units Available</label>
                                    <input type="number" name="units" min={1} value={form.units} onChange={handleChange}
                                        className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 font-bold text-gray-900 focus:outline-none focus:border-blue-500 transition-all" />
                                </div>
                                <div>
                            ...
                            <button type="submit" disabled={submitting}
                                className="w-full bg-blue-600 text-white py-4 rounded-2xl font-semibold uppercase tracking-widest shadow-xl shadow-blue-200 hover:bg-blue-700 transition-all disabled:opacity-50">
