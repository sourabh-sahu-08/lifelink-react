...
const Navbar = ({ onNewRequest, onEmergency }) => {
    ...
    return (
        <>
            ...
            <div className="fixed bottom-6 left-4 right-4 z-[100] lg:hidden pointer-events-auto">
                <nav className="glass-nav rounded-[2rem] px-6 py-3.5 flex items-center justify-around shadow-2xl">
                    {[
                        { name: 'Dashboard', path: '/', icon: 'grid-alt' },
                        { name: 'Requests', path: '/requests', icon: 'list-ul' },
                        { name: 'Profile', path: '/profile', icon: 'user' }
                        ...
                        >
                            <i className={`bx bx-${link.icon} text-xl`}></i>
