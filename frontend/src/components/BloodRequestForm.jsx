                    ...
                    <motion.div initial={{ scale: 0.9, y: 20, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.9, y: 20, opacity: 0 }} className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 z-10">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                ...
                                <p className="text-xs text-gray-400 font-semibold uppercase tracking-widest mt-1">Post an emergency blood need</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Blood Type Needed *</label>
                                <div className="grid grid-cols-4 gap-2">
                                    {BLOOD_TYPES.map(bt => (
                                        <button type="button" key={bt} onClick={() => setForm(p => ({ ...p, bloodType: bt }))}
                                            className={`py-3 rounded-2xl font-semibold text-sm border-2 transition-all ${form.bloodType === bt ? 'bg-red-600 text-white border-red-600 shadow-lg shadow-red-200' : 'border-gray-100 text-gray-500 hover:border-red-300'}`}>
                                            {bt}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="units" min={1} max={20} value={form.units} onChange={handleChange}
                                        className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 font-bold text-gray-900 focus:outline-none focus:border-red-500 transition-all" />
                                </div>
                                <div>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Reason / Notes</label>
                            ...
                            <button type="submit" disabled={submitting}
                                className="w-full bg-red-600 text-white py-4 rounded-2xl font-semibold uppercase tracking-widest shadow-xl shadow-red-200 hover:bg-red-700 transition-all disabled:opacity-50">
